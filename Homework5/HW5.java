...
public class MyDequeUsingStacksCopy<T> {
    ...
    private void move (ResizingArrayStack<T> from, ResizingArrayStack<T> to) {
        ...
        // Whichever 'to' and 'from' are, they should always have reverse orders.
        if (!to.isEmpty ()) throw new IllegalArgumentException();
        if (from.isEmpty ()) throw new IllegalArgumentException();
        int half = from.size() / 2;
        ResizingArrayStack<T> tmp = new ResizingArrayStack<T>();
        // reverses the order of the elements in 'from'
        while (!from.isEmpty())
            tmp.push (from.pop ());
        // puts half back into from in the right order
        while(tmp.size() > half)
            from.push(tmp.pop());
        // puts from into to in reverse order
        while (!from.isEmpty())
            to.push(from.pop());
        // puts the rest back into from in the right order
        while (!tmp.isEmpty())
            from.push(tmp.pop());
    ...
    static double average(ResizingArray<Double> a) {
        double sum = 0;
        for (double item : a) {
            sum += item;
        }
    ...
    public static void main (String args[]) {
        ...
        ResizingArray<Double> ratios = new ResizingArray<>();
